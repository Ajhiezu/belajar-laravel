    ...
    <div class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow">
        ...
        <form action="{{ isset($barang) ? route('barang.update',$barang->id) : route('barang.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="stok" class="w-full border-gray-300 rounded-lg p-2 shadow-sm" value="{{ old('stok', $barang->stok ?? "") }}" required>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Harga Beli</label>
                <input type="number" name="harga_beli" class="w-full border-gray-300 rounded-lg p-2 shadow-sm" value="{{ old('harga_beli', $barang->harga_beli ?? "") }}" required>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Harga Jual</label>
                <input type="number" name="harga_jual" class="w-full border-gray-300 rounded-lg p-2 shadow-sm" value="{{ old('harga_jual', $barang->harga_jual ?? "") }}" required>
            </div>
