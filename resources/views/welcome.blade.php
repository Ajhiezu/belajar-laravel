...
<body class="bg-gray-100 p-6">
    <div class="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow">
        <div class="flex justify-between items-center mb-5">
            ...
            <a href="{{ route('barang.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow">
                + Tambah Barang
            </a>
        </div>
            ...
            <tbody>
                @foreach ($barangs as $barang)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3">@if ($barang->gambar)
                            <img src="{{ asset('uploads/barang/' . $barang->gambar) }}" alt="Gambar" style="width: 60px; height: 60px; object-fit: cover; border-radius: 5px;">
                        @else
                        ...
                        <td class="p-3">
                            ...
                            <form action="{{ route('barang.destroy',$barang->id) }}" method="POST" style="display: inline-block">
                                @csrf
                                @method('DELETE')
                                <button onclick="return confirm('Yakin hpus')" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm">
                                    Hapus
                                </button>
                            </form>
                        </td>
